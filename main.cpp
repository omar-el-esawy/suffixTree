...
#include <cstring>
/*
Omar Mohamed Mostafa 20200359
Habiba Ahmed Mohamed 20200139
Sarah Rady Sayed 20200735
Ibrahim Ismail Ibrahim 20200828
Nada Mohamed Aref 20200591
*/
...
struct Node {
    ...
    {
        ind = (ind == 36 ? 'z' - 'a' + 1 : ind - 'a'); //return 26 if char is $
        return children[ind];
    }
...
public:
    ...
    void build(Node *par, int ind, int orignal_ind)
    {
        ...
        if (len == opar-63>get_len(sz)) //if the common length is equal to the parent length
        {
        ...
        if (opar->node_size != 0) //if the old parent had children we will keep them as the new parents children
        {
            ...
            for (int i = 0; i < 27; i++) //add the old parent children to the lead node
            {
                if (opar->children[i] != nullptr)
                {
                    lead->children[i] = opar->children[i];
                }
            }
